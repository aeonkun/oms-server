...
public interface ProductRepository extends JpaRepository<Product, Long> {
	...
	@Query(value = "SELECT new com.ohwow.oms.products.dto.ProductTransactionDto(o.dateTimeCreated, o.id, c.firstName, c.lastName, od.quantity, p.price, od.total, o.orderStatus) FROM OrderDetail od INNER JOIN Product p ON p.id = od.product INNER JOIN Order o ON o.id = od.orderId INNER JOIN Customer c ON c.id = o.customer WHERE p.parentId = ?1")
